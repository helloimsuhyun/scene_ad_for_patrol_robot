...
지금 깨지는 이유는 README에 코드블록(```)과 일반 텍스트가 섞이면서 마크다운 렌더링이 꼬인 것 때문이야.
특히 숫자 결과를 전부 코드블록으로 감싸서 GitHub에서 보기 안 좋아지고 줄 정렬도 깨져.

아래는 GitHub에서 안 깨지고 바로 보이는 정리된 버전으로 다시 정리한 거야. 그대로 덮어쓰면 됨.
...
## 🏭 자체 데이터셋 실험 결과 (Patrol Robot Environment)

실제 순찰 환경에서 수집한 이미지 데이터(`place=00`)에 대해 동일한 파이프라인을 적용하여 성능을 평가하였다.
...
총 120장의 이미지(정상 67, 이상 53)에 대해 평가하였다.

분석 결과, 실제 순찰 환경의 해당 시스템 도입에 대한 가능성 Accuracy: **90.83%**를 확인하였다.
