...
def run_calibration(
    bank_dir: Path,
    th_calib_dir: Path,   # ← 캘리브레이션용 정상 이미지 폴더 (th_calib/)
    out_dir: Path,
    sg,
    local_model,
    device: str,
    local_tfm,
    cfg: dict,
    plc_idx: str = "",       # threshold.json의 plc_idx 필드용
    radius: int = 1,
    calib_method: str = "robust",
    calib_k: float = 3.0,
    calib_max_imgs: int = 30,   # th_calib 이미지 캘리브레이션에 사용할 최대 수
    calib_n_ref: int = 5,       # [Fix1] 이미지당 bank 중 시도할 후보 수 (best-match)
    seed: int = 0,
    dino_model=None,            # DINO preselection 모델 (None이면 랜덤 폴백)
    dino_tfm=None,              # DINO 전처리 transform
    bank_dino: dict = None,     # build_dino_bank() 결과 (None이면 랜덤)
    dino_top_m: int = 5,        # DINO preselection 후보 수
) -> dict:
    """
    [케리브레이션 전략 — Fix1 best-match 방식]
    th_calib 이미지마다 bank 중 calib_n_ref장을 시도하여
    min(score)만 수집 → 이거이 infer와 동일한 best-match 분포.

    - calib_max_imgs: th_calib 이미지 중 최대 사용 수 (O(N) 억제)
    - calib_n_ref  : 이미지당 bank 후보 수, infer의 n_ref_candidates와 맞춰서 사용 권장
    """
    random.seed(seed)
    ...
    # [Fix1] th_calib 이미지 하나당 bank calib_n_ref장을 시도하고 min(score) 수집
    # → infer의 best-match 방식과 동일한 분포 학습
    random.shuffle(th_paths)
    use_th = th_paths[:calib_max_imgs]  # th_calib 이미지 코드 수 제한

    scores = []      # [component-level] 모든 정상 component 점수 수집
    pair_log = []
    ...
    for th_idx, q_path in enumerate(use_th):
        ...
        if dino_model is not None and dino_tfm is not None:
            ...
            if len(bank_candidates) == 0:
                # DINO 실패 시 랜덤
                shuffled = bank_paths.copy(); random.shuffle(shuffled)
                bank_candidates = shuffled[:calib_n_ref]
        else:
            # DINO 없으면 기존 랜덤 방식
            shuffled_banks = bank_paths.copy()
            random.shuffle(shuffled_banks)
            bank_candidates = shuffled_banks[:calib_n_ref]
        ...
        for r_path in bank_candidates:
            ...
            if score is None:
                print(f"  [CALIB-SKIP] {q_path.name} ↔ {r_path.name}: "
                      f"{debug.get('reason')}")
                continue

            cand_scores.append(float(score))
            cand_infos.append({"r": r_path.name, "score": float(score),
        ...
        # [Fix1] best-match: 정상 쿼리에 가장 변화 없는 ref 선택
        best_idx  = int(np.argmin(cand_scores))
        best_info = cand_infos[best_idx]

        # 각 정상 이미지에서의 "최대 정상 component 점수"만 수집
        ...
        print(f"  [CALIB] {th_idx+1}/{len(use_th)} {q_path.name} "
    ...
    thr_json = {
        "plc_idx"    : plc_idx,
        "repr_mode"  : "compound_cc",          # 이 파이프라인의 repr 식별자
        "k"          : float(calib_k),
        "method"     : calib_method,
    ...
    thr_path.write_text(
        json.dumps(thr_json, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    print(f"\n[CALIB] threshold.json 저장 완료: {thr_path}")
    print(f"[CALIB] threshold={thr_json['threshold']:.4f}, num_th={thr_json['num_th']}")

    # pair 로그 저장
    (out_dir / "calib_pair_log.json").write_text(
        json.dumps(pair_log, indent=2, ensure_ascii=False), encoding="utf-8"
    )
...
def verify_bbox_with_local_search(
    ...
    r_region = to_gray_clahe_3ch(r_region)
...
def _save_summary_gallery(out_dir: Path):
    ...
    for case_dir in sorted(out_dir.iterdir()):
        ...
        overlay_path = None
        candidates = ["sam_overlay_q.png", "compound_best_component.png", "q_crop.png"]
        for c in candidates:
            p = case_dir / c
            if p.exists():
                overlay_path = p
...
def main():
    ...
    # 캘리브레이션 전용 인자
    parser.add_argument("--calib_method",    default="robust",
                        choices=["robust", "gaussian", "percentile"],
                        help="threshold 계산 방식 (robust=MAD 기반, gaussian=평균±k*σ)")
    parser.add_argument("--calib_k",     type=float, default=3.0,
                        help="3-sigma 배율 (기본 3.0)")
