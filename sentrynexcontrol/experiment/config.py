...
def load_cfg(bank_root: Path) -> dict:
    ...
    # 하위호환: repr_modes 블록이 일부 비어 있어도 patchcore/calib 기본값으로 채움
    gpa = out.setdefault("repr_modes", {}).setdefault("global_patch_with_aligned", {})

    gpre = gpa.setdefault("global_preselect", {})
    gpre.setdefault("top_m", out.get("patchcore", {}).get("preselect_m", 3))

    cc = gpa.setdefault("cc", {})
    pc = out.get("patchcore", {})
    cc.setdefault("radius", pc.get("radius", 1))
    cc.setdefault("top_p", pc.get("top_p", 0.05))
    cc.setdefault("alpha", pc.get("alpha", 0.6))
    cc.setdefault("min_cut", pc.get("min_cut", 0.2))
    cc.setdefault("singleton_weight", pc.get("singleton_weight", 0.25))
    cc.setdefault("component_min_area", pc.get("component_min_area", 2))

    proposal = gpa.setdefault("proposal", {})
    proposal.setdefault("top_k", 3)
    ...
    calib.setdefault("cc_k", out.get("calib", {}).get("percentile", 97))
    calib.setdefault("final_k", out.get("calib", {}).get("percentile", 97))
    calib.setdefault("final_threshold_floor", 0.45)
    calib.setdefault("max_imgs", 100)
