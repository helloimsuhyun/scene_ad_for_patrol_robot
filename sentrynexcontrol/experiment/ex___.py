...
def run_calibration(
    ...
    for th_idx, q_path in enumerate(use_th):
        ...
        best_comp = max(all_comps, key=lambda c: float(c.get("score", 0.0)))
        flagged_comps = [best_comp]
...
def run_inference(
    ...
    for q_path in query_paths:
        ...
        reject_by_verifier = cc_is_anomaly and (best_verifier_score < verifier_thr)
        is_anomaly = cc_is_anomaly and (not reject_by_verifier)
