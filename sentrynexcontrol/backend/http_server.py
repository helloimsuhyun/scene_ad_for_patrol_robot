...
# 1. 플러터 통신을 위한 CORS (Cross-Origin Resource Sharing) 설정
from fastapi.middleware.cors import CORSMiddleware
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 플러터 개발 환경(웹/에뮬레이터)에서 찔러보기 위해 모든 도메인 오픈
    allow_credentials=True,
    allow_methods=["*"],  
    allow_headers=["*"],
)

# 2. GUI 용 정적(이미지) 파일 서빙 마운트
from fastapi.staticfiles import StaticFiles
app.mount("/images", StaticFiles(directory=str(SAVE_ROOT.absolute())), name="images")

# 3. 플러터 연동용: 이벤트 폴링 전체 조회 API
from fastapi import Query
@app.get("/events")
async def get_events_for_gui(since: Optional[str] = Query(None, description="ISO8601 Timestamp")):
    async with app.state.db_lock:
        cur = app.state.db.cursor()
        
        # 'since' 시간 이후의 이벤트만 조회 (없으면 최근 50개)
        if since:
            cur.execute(
                "SELECT * FROM events WHERE captured_at > ? ORDER BY captured_at DESC LIMIT 50", 
                (since,)
            )
        else:
            cur.execute("SELECT * FROM events ORDER BY captured_at DESC LIMIT 50")
            
        events_rows = cur.fetchall()
        
        # 각 이벤트별로 frames 정보 합치기
        result_events = []
        for row in events_rows:
            ...
            frames_list = []
            for f in frames_rows:
                f_dict = dict(f)
                raw_path = Path(f_dict["image_path"]).as_posix()
                # GUI 클라이언트(Flutter)에서 http://~/images/장소/경로 로 바로 접근 가능하도록 가공
                serve_path = raw_path.split("recv/")[-1] if "recv/" in raw_path else Path(f_dict["image_path"]).name
                f_dict["image_path"] = serve_path
                frames_list.append(f_dict)
                
            event_dict["frames"] = frames_list
...
if __name__ == "__main__":
    import uvicorn
    # 외부 기기(로봇 등)가 접속하려면 0.0.0.0 으로 열어주어야 합니다.
