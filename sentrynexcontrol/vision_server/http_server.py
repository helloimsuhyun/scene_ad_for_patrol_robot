...
PERSON_EVENT_ROOT = Path("./recv_person") # 사람 감지 이미지 root
...
class CreatePlaceReq(BaseModel):
    place_id: str
    display_name: str
    x: float
    y: float
...
@app.post("/places")
async def create_place_api(req: CreatePlaceReq):
    async with app.state.db_lock:
        place = sqlite_db.create_place(
            app.state.db, 
            req.place_id, 
            req.display_name, 
            req.x, 
            req.y, 
            req.yaw, 
            1 if req.patrol_enabled else 0
        )
    return {"ok": True, "place": place}
