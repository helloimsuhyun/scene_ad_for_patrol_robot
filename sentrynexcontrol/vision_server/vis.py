...
# 만약 warp_utils에서 crop_common_safe_region으로 완전히 바꿨다면
# 위 import를 아래로 바꿔도 된다.
# from .warp_utils import warp_query_to_bank, crop_common_safe_region


# --------------------------------------------------
# basic helpers
# --------------------------------------------------
def _as_u8_bgr(x) -> np.ndarray:
...
def to_cnn_view_bgr_from_bgr(img_bgr: np.ndarray, img_size: int = 560) -> np.ndarray:
    """
    aligned local CNN 입력과 같은 spatial view를 만들기 위한 helper.
    현재는 resize-only 기준.
    local transform이 pad 방식이면 여기도 동일하게 맞춰야 한다.
    """
    if img_bgr is None:
        raise ValueError("img_bgr is None")
...
def draw_top_p_heatmap(
    ...
    if normalize_each:
        ...
        if abs_min is None or abs_max is None:
            raise ValueError("normalize_each=False requires abs_min, abs_max")
        if abs_max <= abs_min:
...
def save_aligned_debug_vis(
    ...
    우선순위:
      1) align_vis["data"] 안에 vis_query_bgr / vis_ref_bgr 가 있으면 그걸 그대로 사용
      2) 없으면 기존처럼 warp/crop 재구성 fallback
    """
    if not align_vis:
        return
    ...
    vis_query_bgr = data.get("vis_query_bgr", None)
    vis_ref_bgr = data.get("vis_ref_bgr", None)
    ...
    # 1) distance 실제 입력 이미지가 있으면 그걸 바로 사용
    if vis_query_bgr is not None and vis_ref_bgr is not None and grid_hw is not None:
        vis_query_bgr = _as_u8_bgr(vis_query_bgr)
        vis_ref_bgr = _as_u8_bgr(vis_ref_bgr)

        overlay_img = make_overlay(vis_query_bgr, vis_ref_bgr, alpha=0.5)

        vis_img = draw_top_p_heatmap_aligned(
            img_bgr=vis_query_bgr,
        ...
        cv2.imwrite(str(out_prefix.parent / f"{out_prefix.name}_01_query_aligned.jpg"), vis_query_bgr)
    ...
    warped_q_crop_cnn = to_cnn_view_bgr_from_bgr(warped_q_crop, img_size=img_size)
    ref_crop_cnn = to_cnn_view_bgr_from_bgr(ref_crop, img_size=img_size)

    overlay_img = make_overlay(warped_q_crop_cnn, ref_crop_cnn, alpha=0.5)

    vis_img = draw_top_p_heatmap_aligned(
        img_bgr=warped_q_crop_cnn,
        top_patch_idx=top_patch_idx,
        top_patch_vals=top_patch_vals,
        grid_hw=grid_hw,
        alpha=0.45,
    ...
    cv2.imwrite(str(out_prefix.parent / f"{out_prefix.name}_00_query_raw.jpg"), query_raw)
    cv2.imwrite(str(out_prefix.parent / f"{out_prefix.name}_01_query_aligned.jpg"), warped_q_crop_cnn)
    cv2.imwrite(str(out_prefix.parent / f"{out_prefix.name}_02_query_ref_overlay.jpg"), overlay_img)
    cv2.imwrite(str(out_prefix.parent / f"{out_prefix.name}_03_query_aligned_crop_heatmap.jpg"), vis_img)
...
def save_patch_match_vis(
    ...
    if repr_mode == "global_patch_with_aligned":
        ...
        vis_query_bgr = patch_vis.get("vis_query_bgr", None)
        vis_ref_bgr = patch_vis.get("vis_ref_bgr", None)

        # 1) distance 실제 입력 이미지가 있으면 그걸 우선 사용
        if vis_query_bgr is not None and vis_ref_bgr is not None and grid_hw is not None:
            vis_query_bgr = _as_u8_bgr(vis_query_bgr)
            vis_ref_bgr = _as_u8_bgr(vis_ref_bgr)

            vis_img = draw_patch_match_lines_aligned(
                query_bgr=vis_query_bgr,
                ref_bgr=vis_ref_bgr,
            ...
            vis_img = draw_patch_match_lines_aligned(
                query_bgr=warped_q_crop_cnn,
                ref_bgr=ref_crop_cnn,
                query_patch_idx=top_patch_idx,
                ref_patch_idx=top_patch_match_idx,
                query_patch_vals=top_patch_vals,
                query_grid_hw=grid_hw,
