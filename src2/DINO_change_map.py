...
def load_dinov2(model_name="vit_base_patch16_dinov3.lvd1689m", device=None):
...
def extract_patch_tokens(model, x): #model에서 patch 토큰을 추출
    pt_all = model.forward_features(x)  # timm model output

    # pt_all이 [1, N, D]일 때만 처리
    if pt_all.dim() != 3:
        raise RuntimeError(f"Unexpected feature shape: {tuple(pt_all.shape)}")
    ...
    if ps is None or not hasattr(model.patch_embed, "patch_size"):
        raise RuntimeError("Cannot infer patch_size from model.patch_embed.patch_size")
    patch_size = model.patch_embed.patch_size
    patch_size = patch_size[0] if isinstance(patch_size, (tuple, list)) else int(patch_size)

    H, W = x.shape[-2], x.shape[-1]
    Hp0, Wp0 = H // patch_size, W // patch_size
    P0 = Hp0 * Wp0

    # 뒤에서 P0개만 patch로 간주 (앞쪽은 cls/dist/reg 같은 extra token)
    # => 토큰 개수 mismatch(1604 vs 1600) 같은 케이스를 자동으로 해결
    if pt_all.shape[1] < P0:
        raise RuntimeError(f"Token length {pt_all.shape[1]} < expected patches {P0}")

    pt = pt_all[0, -P0:, :]  # [P0, D]

    return pt, Hp0, Wp0, patch_size
...
def postprocess_change_map(m):
    ...
    """
    raw = m.copy()
    raw = constant_threshold_keep_value(raw, 0.1)
...
def pick_local_maxima_topk(change_map, topk=3, k=3, neigh=11, min_dist=96,
                           min_percentile=90, abs_floor=0.0, patch_size=16):
...
def compute_change_map(
    ...
    return fused_up, overlay_q, overlay_r, score, q_rgb_518 , r_rgb_518, patch_size
...
def draw_rois_on_image(rgb, rois, color=(0, 255, 0), thickness=2):
    ...
    for r in rois:
        ...
        label = f"id{r['id']} dino={r['score']:.3f}"
...
if __name__ == "__main__":
    query = "/home/choisuhyun/scene_ad_for_patrol_robot/data/00/003.png"
    ref   = "/home/choisuhyun/scene_ad_for_patrol_robot/data/00/000.png"
    
    img_size = 640  # 16의 배수로
    model_name = "vit_base_patch16_dinov3.lvd1689m"

    #dino map
    change_map, overlay_q, overlay_r, score, q_rgb_518, r_rgb_518, patch_size = compute_change_map(
        query_path=query,
        ref_path=ref,
    ...
    peaks = pick_local_maxima_topk(change_map,topk=3,k=3,patch_size=patch_size)
